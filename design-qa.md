...
- Real project images now sit on a shared "media plate": a hairline-bordered, softly shadowed, rounded container with a fixed aspect ratio, `object-fit: contain` so no capture is cropped sideways, and a short bottom fade so partial-page captures do not read as bad crops. This supersedes the earlier standalone/unframed treatment.
