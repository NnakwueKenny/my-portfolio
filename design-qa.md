...
- All project detail routes now use a shared editorial case-study template with an unconstrained full-width product capture, project facts, three indexed content sections, icon-led technology list, and visual next-project handoff.
...
- Project-template verification: Credway and Skybod CarePass both render the shared three-section template, real full-width images with `object-fit: contain`, and their correct technology icons; no horizontal overflow at 1416-pixel desktop or 371-pixel mobile content widths.
